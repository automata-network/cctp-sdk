...
function generateActions(configsSet: CCTPSdkConfigsSet) {
  return {
    ...
    isTransferCompleted: (
      options: Omit<IsTransferCompletedOptions, "configsSet">
    ) => {
      return isTransferCompleted({ configsSet, ...options });
    },
  };
}
...
export interface TransferUSDCOptions {
  ...
  destinationSigner?: Signer | providers.JsonRpcSigner;
  onApprove?: () => void;
  onBurnUSDC?: () => void;
...
async function getMessageBytes(options: GetMessageBytesOptions) {
  const { sourceDomain, burnTxHash, configsSet } = options;
  const networkConfigs = getNetworkConfigs(configsSet, sourceDomain);
  const burnTx = await getEthTransactionReceipt({
    rpc: networkConfigs.rpc,
    txHash: burnTxHash,
  });
...
export interface IsTransferCompletedOptions {
  sourceDomain: number;
  burnTxHash: string;
  configsSet: CCTPSdkConfigsSet;
}

async function isTransferCompleted(options: IsTransferCompletedOptions) {
  const { sourceDomain, burnTxHash, configsSet } = options;
  const networkConfigs = getNetworkConfigs(configsSet, sourceDomain);
  const burnTx = await getEthTransactionReceipt({
    rpc: networkConfigs.rpc,
  ...
  const logs = parseLog(iface, burnTx.logs, "DepositForBurn");
  const nonce = logs?.parsedLog.args.nonce;
  const destinationDomain = logs?.parsedLog.args.destinationDomain;
  const destinationNetworkConfigs = getNetworkConfigs(
    configsSet,
    destinationDomain
  );
  const contract = getMessageTransmitterContract(
    destinationNetworkConfigs.cctpMessageTransmitterContractAddress,
    new providers.StaticJsonRpcProvider(destinationNetworkConfigs.rpc)
  );
  const hashedSourceAndNonce = solidityKeccak256(
    ["uint32", "uint64"],
    [sourceDomain, nonce.toNumber()]
  );
  const isNonceUsed = await contract.usedNonces(hashedSourceAndNonce);

  return isNonceUsed.toNumber() === 1;
}
